...
func UpdateProduct(c *fiber.Ctx) error {
	...
	go database.CleanCache("products_frontend", "products_backend")
...
func ProductBackend(c *fiber.Ctx) error {
	...
	if s := c.Query("s"); s != "" {
		...
		for _, product := range products {
			if strings.Contains(strings.ToLower(product.Title), lower) || strings.Contains(strings.ToLower(product.Description), s) {
